...
import com.xrandolphx.elegantcommerce.ElegantCommerce
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?) {
        ...
        if (!areInputsValid) {
            viewModelScope.launch {
                _user.emit(Resource.Error("Revisa el contenido ingresado"))
            }
            return
        }
            ...
            try {
                val imageBitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    val source = ImageDecoder.createSource(
                        getApplication<ElegantCommerce>().contentResolver,
                        imageUri
                    )
